                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td class="md:px-4 py-4 text-center">
                                  <button onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id]) }}'" class="text-white bg-yellow-500 border-0 py-2 md:px-6 px-3 focus:outline-none hover:bg-yellow-600 rounded">編集</button>
                                </td>
                                <form id="delete_{{ $owner->id }}" method="post" action="{{ route('admin.owners.destroy', ['owner' => $owner->id]) }}">
                                  @method('delete')
                                  @csrf
                                  <td class="md:px-4 py-4 text-center">
                                    <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class="text-white bg-red-500 border-0 py-2 md:px-6 px-3 focus:outline-none hover:bg-red-600 rounded">削除</a>
                                  </td>
                                </form>
                              </tr>
                              @endforeach
