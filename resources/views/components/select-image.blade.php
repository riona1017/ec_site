    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
        <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="{{ $modal }}-title">
                        ...
                        <div class="w-1/2 md:w-1/4 p-2 md:p-4">
                            <div class="border rounded-md p-2 md:p-4">
                                <img class="image" data-id="{{ $name }}_{{ $image->id }}"
                                    ...
                                    data-micromodal-close="{{ $modal }}"
                                    src="{{ asset('storage/products/' . $image->filename) }}">
                                <div class="mt-2 text-gray-700">{{ $image->title }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <footer class="modal__footer">
                <button id="{{ $modal }}_close" type="button" class="modal__btn" data-micromodal-close aria-label="Close this dialog window">閉じる</button>
            </footer>
        </div>
    </div>
</div>
...
<div class="flex justify-around items-center mb-4">
    <a class="bg-gray-100 border-0 py-2 px-8 mb-4 focus:outline-none hover:bg-gray-300 rounded" data-micromodal-trigger="{{ $modal }}" href='javascript:;'>ファイルを選択</a>
    <div class="w-1/4">
